...
export class SidebarComponent implements OnInit, AfterViewInit {
  ...
  ngOnInit(): void {
    window.addEventListener('resize', (e: any) => {
      ...
      this.setMenuWidth(e.target.innerWidth - 100);
      // console.table({"targetInnerWidth":e.target.innerWidth,"targetOuterWidth":e.target.outerWidth,"currentTareget":e.target.innerWidth,"currentTaregetOuterWidth":e.target.outerWidth});
  ...
  ngAfterViewInit() {
    ...
    this.menuState.firstLevelFullWidth = document.getElementsByClassName('level1')[0].clientWidth + 'px';

    setTimeout(() => {
      this.menuState.firstLevelShowFullWidth = false;
this.setMenuWidth(window.innerWidth - 100);
    }, 100);

  }


  setMenuWidth(windowWidth: number,type = 0) {
    if (type == 0)
    {
    ...
    if (windowWidth <= 500) {
      ...
      if (this.menuState.first_level_expanded) {
        this.menuState.firstLevelMobileWidth = Math.round(windowWidth / 2) + 'px';
        this.menuState.firstLevelWidth = (parseInt(this.menuState.firstLevelMobileWidth) <  parseInt(this.menuState.firstLevelFullWidth))?this.menuState.firstLevelMobileWidth:this.menuState.firstLevelFullWidth;
        
        let ele:any = document.getElementsByClassName('level2')[0];
        if (parseInt(this.menuState.firstLevelMobileWidth) + document.getElementsByClassName('level2')[0].clientWidth > windowWidth)
        {
         ele.style.width = Math.round(windowWidth / 2).toString()+"px";
  
        }
      }
      else {
        this.menuState.firstLevelWidth = '36px';
        let ele:any = document.getElementsByClassName('level2')[0];
        ele.style.removeProperty('width');
        // this.menuState.secondLevelPosition = 
      }
      ...
      this.menuState.firstLevelMobileWidth = '';
      let ele:any = document.getElementsByClassName('level2')[0];
      ele.style.removeProperty('width');
  ...
  setMenuWidthOnAnimation(windowWidth:number,type=0)
  {
    if (windowWidth <= 500)
    {
      if (type == 2)
      {
        // if (wind)
        // let ele:any = document.getElementsByClassName('level1')[0];
      }
    }
  }
  ...
  toggleMain(val = 0) {
    ...
    if (!this.menuState.first_level_expanded) {
      ...
      this.setMenuWidth(window.innerWidth);
  ...
  toggleLevelTwo(menuItem, state,event) {
    
    let elems = document.getElementsByClassName('level1')[0].getElementsByTagName('ul')[0].querySelectorAll("li");
    elems.forEach((item)=>{
    
      item.classList.remove('level1-active');
    });
    let element =  event.target;
    if(element.tagName.toLowerCase() === 'a')
    {
    
     element.parentElement.classList.add("level1-active");
    }
    else 
    {
      element.parentElement.parentElement.classList.add("level1-active")
    }
    if (menuItem.links) {
  ...
  onAnimationEvent(event) {
    if (event.triggerName == "toggleWidth" && event.toState == true && event.phaseName == "done") {
      this.menuState.firstLevelShowFullWidth = true;      
        this.menuState.showText = true;
      
    
    }

    else if (event.triggerName == "toggleWidth" && event.phaseName == "start" && event.toState == false)
    {
      if (this.menuState.firstLevelMobileWidth !='')
      {
        this.menuState.showText = false;
      }
    }
    ...
    else if (event.triggerName == "toggleWidth" && event.phaseName == "start" && event.toState == true) {
      this.menuState.firstLevelShowFullWidth = true;
  ...
  setAnimationPositions(type) {
    ...
    setTimeout(() => {
      ...
      if (!this.menuState.runAnim) {



        document.getElementsByClassName('level2')[0].setAttribute('style', "margin-left:-" + document.getElementsByClassName('level2')[0].clientWidth + "px");
    ...
    setTimeout(() => {
      ...
      if (type == 2) {
        if (!this.menuState.second_level_expanded) {
          ...
          this.menuState.secondLevelPosition = '0px';
          this.menuState.second_level_expanded = true;
          this.menuState.runAnim = true;
        }
